...
public class Unicode extends APIBase {
	...
	public void pushFields() {
		...
		machine.lua.pushJavaFunction(args -> {
			machine.lua.pushBoolean(Glyph.map[args.checkString(1).codePointAt(0)].width / (double) Glyph.WIDTH > 1);
			return 1;
		});
		machine.lua.setField(-2,"isWide");
        ...
        machine.lua.pushJavaFunction(args -> {
            machine.lua.pushString(new StringBuilder(args.checkString(1)).reverse().toString());
            return 1;
        });
        machine.lua.setField(-2,"reverse");
	}
}